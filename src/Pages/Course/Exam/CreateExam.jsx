...
import { createExam } from '../../../Store';
import { useParams } from 'react-router-dom';
import { ToastContainer } from "react-toastify";
const CreateExam = () => {
    const {user:{token}}=useSelector(state=>state.authentication);
    const {isLoding}=useSelector(state=>state.exam);
    const [imageFile , setImageFile]           = useState(null);
    const {id} = useParams();
    const disPatch = useDispatch();
    // createExam
    const [startTime, setStartTime] = useState(new Date());
    const [terminationTime, setTerminationTime] = useState(new Date());
    ...
    function handleSubmit(event){
        ...
        function convert(str) {
            ...
            return [date.getFullYear(), month, day].join("-") +" "  + time;
          }
        const exam ={
            startTime ,
            terminationTime ,
        ...
        formData.append('data', JSON.stringify(exam));
        disPatch(createExam({token,formData,id}));
    }
    ...
    return ( 
        <div className="CreateExam container">
            ...
            <img src={imageFile ? URL.createObjectURL(imageFile) :"https://d20x1nptavktw0.cloudfront.net/wordpress_media/2022/07/blog-image-16.png"} alt="user-img" />
